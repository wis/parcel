...
export default class SimplePackageInstaller implements PackageInstaller {
  fs: FileSystem;
  cache: {|resolve: Map<string, ResolveCacheEntry>, fetch: Map<string, any>|};
  ...
  async _npmResolve(name: string, version: string) {
  ...
  async _npmFetch(tarball: string) {
  ...
  async _getDependency(name: string, version: string) {
    let {
      dependencies: deps,
      dist: {tarball},
